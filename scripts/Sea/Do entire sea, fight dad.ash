void main()
{
	...
	if (have_effect($effect[Fishy]) < 20 && !contains_text(visit_url("sea_merkin.php?seahorse=1"), "you crest an undersea ridge and discover, spread out beneath you, a magnificent Mer-kin City"))
	{
	...
	cli_execute("mood none");
	cli_execute("call Sea/Level to 150-150-150.ash");
	cli_execute("familiar adorable space buddy");
	cli_execute("call Sea/Fight Dad Sea Monkee.ash");
	if (available_amount($item[pocket square of loathing]) > 2)
	{
		print("should pulverize pocket square of loathing");
	}
	cli_execute("mood none");
}